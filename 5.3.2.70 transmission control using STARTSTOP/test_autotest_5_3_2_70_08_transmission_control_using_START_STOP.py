"""
То же самое, что и в предыдущем тесте, но с отправкой Серверу S-кадра
"""
...
def test_autotest_5_3_2_70_08_trans_con_us_START_STOP(IP,PORT,t1_SERVER):
...
### Проверка STARTDT_ACT
    ...
    if i >= t1_SERVER + 1:
        testcon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.08: !Ошибка:     После отправки команды STARTDT_ACT клиентом не получен STARTDT_CON"
...
### Проверка STOPDT_ACT
    ...
    if i >= t1_SERVER + 1:
        testcon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.08: !Ошибка:     После отправки команды STOPDT_ACT клиентом не получен STOPDT_CON"
        log(FILENAME,testcon)
        assert str(testfr[-1].data.type) == "STOPDT_CON", "Не получен STOPDT_CON"

# Проверить наличие события CONNECTION_CLOSED
    client.sendSFrame()
    ...
    if i >= 2:
        closecon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.08: !Ошибка:     Сервер не разорвал TCP соединение после отправки S кадра в Stopped connection (после Started connection)"
        log(FILENAME,closecon)
        assert i <= 2, "Сервер не разорвал TCP соединение после отправки I кадра"


    closecon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.08: Пройдено:     Сервер разорвал TCP соединение после отправки S кадра в Stopped connection (после Started connection)"
    log(FILENAME,closecon)