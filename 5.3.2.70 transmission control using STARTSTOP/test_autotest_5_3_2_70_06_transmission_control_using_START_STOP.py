# Оставить ожидание STARTDT_CON и SOPDT_CON максимум t1 с
"""
Проверка U-кадрами TESTFR в состоянии Stopped Connection после завершения обмена данными
...
4. Сервер генерирует 2 I-кадра
5. Ожидаем, что Клиент не получил I-кадры
6. Отправляем TESTFR_ACT, ожидаем TESTFR_CON
7. Проверяем, что TCP соединение не разорвано
"""
import rkts
import os
import time
...
def test_autotest_5_3_2_70_06_trans_con_us_START_STOP(IP,PORT,t1_SERVER):
...
### Проверка STARTDT_ACT
    ...
    if i >= t1_SERVER + 1:
        testcon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: !Ошибка:     После отправки команды STARTDT_ACT клиентом не получен STARTDT_CON"
...
### Проверка STOPDT_ACT
    ...
    if i >= t1_SERVER + 1:
        testcon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: !Ошибка:     После отправки команды STOPDT_ACT клиентом не получен STOPDT_CON"
        log(FILENAME,testcon)
    ...
    if len(recv_I_frame) != 0:
        Irec = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: !Ошибка:     Получены I кадры после STOPDT_CON"
        log(FILENAME,Irec)
        assert len(recv_I_frame) == 0, "Получены I кадры после STOPDT_CON"
...
### Проверка на TESTFR
    while (str(testfr[-1].data.type) != "TESTFR_CON") and (i <= t1_SERVER + 1):
        testfr = u_frame_checker(client)
        time.sleep(0.2)
    ...
    if i >= t1_SERVER + 1:
        testcon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: !Ошибка:     Не получен TESTFR_CON"
        log(FILENAME,testcon)
        assert str(testfr[-1].data.type) == "TESTFR_CON", "Не получен TESTFR_CON"

    for ev in client.events():
        if ev.type == rkts.EventType.CONNECTION_CLOSED:
            closecon = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: !Ошибка:     Сервер разорвал TCP соединение после отправки TESTFR"
    ...
    contest = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.70.06: Пройдено:     Получен TESTFR_CON, обмен U кадров в Stopped connection(после Started connection), I кадры не получены"
