...
def test_timeout3_autotest_5_3_2_90_11(t3_SERVER, det,IP,PORT):
    bufferclear(IP,PORT)
    ...
    Itimert3 = client.events()[-1].timestamp/1000
    time.sleep(t3_SERVER + det)

    if str(u_frame_checker(client)[-1].data.type) == "TESTFR_ACT":
        Sctimet3act = u_frame_checker(client)[-1].timestamp/1000
        Sctimet3 = int(Sctimet3act - Itimert3) 
    else:
        wart3 = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.11: !Ошибка:     После таймера t3 не обнаружен TESTFR_ACT"
        log(FILENAME,wart3)
        assert str(u_frame_checker(client)[-1].data.type) == "TESTFR_ACT", "Проверка таймера t3"

    if t3_SERVER - 1 <= Sctimet3 <= t3_SERVER + 1:
        Sct3poss = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.11: Пройдено:     Сброс таймера t3 I кадром сервера"
        log(FILENAME,Sct3poss)
    else:
        t3neg = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.11: !Ошибка:     Таймер t3 не сбросился после отправки I кадра \n"
        log (FILENAME, t3neg)
        assert t3_SERVER - 1 <= Sctimet3 <= t3_SERVER + 1, "Сброс таймера t3"
        Sct3poss = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.11: Пройдено:     Сброс таймера t3 I кадром сервера"
        log(FILENAME,Sct3poss)