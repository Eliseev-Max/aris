...
def test_timeout1I_autotest_5_3_2_90_2(t1_SERVER, det,IP,PORT):
    bufferclear(IP,PORT)
    t1server = str (t1_SERVER)
...
#### STEP 1. Проверка таймера t1 I frame
#### Проверка работу таймера, проверяем сброс таймера S кадром
    spGenerator(IP,cvalue, 3, 0.3*t1_SERVER)
    time.sleep(0.5)
    ...
    if unconfRecIFrames_0 != 0:
         client.sendSFrame(unconfRecIFrames_0 - 2)
    else:
         errI = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: !Ошибка:     Не получен ни один I кадр от сервера"
         log(FILENAME, errI)
         assert unconfRecIFrames_0 != 0, "Не были получены I кадры"
    ...
    if len(recv_I_frame) < 3:
        assert len(recv_I_frame) >= 3, "Проверка таймера t1 (I Frame), Получены не все сгенерированные I кадры"

    firstIframetime = int(recv_I_frame[-3].timestamp/1000)
    twIframetime = int(recv_I_frame[-2].timestamp/1000)
    ...
    for ev in client.events():
        if ev.type == rkts.EventType.CONNECTION_CLOSED:
            ...
            result11 = conCloseTime - firstIframetime
    ...
    if conCloseTime == 0:
        t1err = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: !Ошибка:     После ожидания таймера t1 не был зафиксирован разрыв соединения (S кадр)"
        log (FILENAME,t1err)
        assert conCloseTime != 0, "Проверка таймера t1 (I Frame)"

    if t1_SERVER - 1 <= result12 <= t1_SERVER + 1:
        t1pos = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: Пройдено:     Корректное срабатывание таймера t1, сброс таймера t1 S кадром"
        log(FILENAME, t1pos)
    else:
        if t1_SERVER - 1 <= result11 <= t1_SERVER + 1:
            t1neg1 = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: !Ошибка:    Таймер t1 не был сброшен после отправки серверу S кадра\n"
            log(FILENAME,t1neg1)
            assert t1_SERVER - 1 <= result11 <= t1_SERVER + 1, "Проверка таймера t1: сброс таймера S кадром"

        if t1_SERVER - 1 <= result13 <= t1_SERVER + 1:
            t1neg1 = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: !Ошибка:    Таймер t1 сработал после последнего полученного I кадра\n"
            log(FILENAME,t1neg1)
            assert t1_SERVER - 1 <= result13 <= t1_SERVER + 1, "Проверка таймера t1: корректная работа"
        
        sresult12 = str(result12)
        t1neg = "\n"+time.strftime("%Y%m%d_%H.%M")+" Autotest 5.3.2.90.2: !Ошибка:    \nПолученное значение таймера t1 ["+sresult12 + " c] не соответствует заданному ["+ t1server +" c])\n"
        log(FILENAME, t1neg)
